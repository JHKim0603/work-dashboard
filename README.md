...
4. **원자재 · 물류 가격** — 각 카드는 **차트 위에 마우스를 올리면 해당 시점의 날짜와 가격**이
   ...
   환율이 맨 앞인 이유: 나머지 카드가 전부 달러(또는 위안) 표시라, 원화 환율이 그 아래 모든
   숫자의 실제 원가를 다시 계산합니다. 최근 2년 1,308~1,557원으로 19% 움직였습니다.
...
- 막대는 24개로 **버킷 평균**해 줄입니다 — N개마다 샘플링하면 그 사이의 급등락이 통째로 사라집니다
