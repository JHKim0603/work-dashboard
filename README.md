...
로컬 실행 **84초 → 약 25초**, GitHub Actions 의 `Generate dashboard.html` 단계 **평소 66초 → 40초**
(캐시가 채워진 실행 기준, 2026-09-26 측정)로 줄였습니다. 요청마다 시간을 재서 원인을 찾은 결과입니다.
페이지 쪽은 손대지 않았습니다 — 압축 56KB, 이 노트북 크롬에서 화면 완성 60ms 로 이미 충분히 가볍습니다.
...
| (CI에서만) 제한 시간 없는 요청이 응답 없이 **296초** 대기 — SSE 운임 한 건, 실행 344초 중 | 모든 웹 요청의 기본 제한 시간을 30초로(`$PSDefaultParameterValues`). 직접 적은 값(GDACS 45초 등)은 그대로 우선. 568초까지 걸리던 실행도 같은 원인으로 보임 |
...
- **막대 개수를 늘릴 때는 메일 용량을 확인하세요.** 막대 하나가 중첩 테이블이라 약 220B이고,
  ...
  붙으며 CI 메일이 약 84KB(83%)까지 올라, 2026-09에 막대 표기를 한 번 더 줄여 **CI 77KB / 75%**
  입니다(로컬은 경유·휘발유 카드가 없어 68KB). 실행 로그에 `Email summary written: ... (77 KB)` 로 찍힙니다
